...
export async function generatePhase0Foundations(input: Input) {
  ...
  const scanResults = scanResultSchema.safeParse(input.scanResults ?? null).success
    ? scanResultSchema.parse(input.scanResults ?? null)
    : null;
  let packetDeckHighlights: string[] = [];
  let brandKit: Awaited<ReturnType<typeof generatePhase0BrandKit>> | null = null;
  ...
  try {
    ...
    packetDeckHighlights = packetDeck.highlights;
    ...
    await log_task(
      ...
      "phase0_packet_deck",
      "failed",
      error instanceof Error ? error.message.slice(0, 240) : "Phase 0 packet deck generation failed",
    ).catch(() => {});
  }
  ...
  try {
    ...
    brandKit = await generatePhase0BrandKit({
    ...
    await log_task(
      ...
      "phase0_brand_foundation",
      "failed",
      error instanceof Error ? error.message.slice(0, 240) : "Brand foundation generation failed",
    ).catch(() => {});
  }
  ...
  await recordProjectEvent(db, {
    projectId: input.projectId,
    eventType: "phase0.summary_ready",
    message: "Phase 0 summary assembled",
  ...
  return {
    deliverables,
    brandKit,
    highlights: packetDeckHighlights,
    techNews: techNews.insight,
    summary,
  };
}
