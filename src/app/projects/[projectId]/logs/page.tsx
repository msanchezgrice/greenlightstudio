...
import type { ProjectAssetRow } from "@/lib/studio";
import { withRetry } from "@/lib/retry";
import { getAgentProfile, humanizeTaskDescription, taskOutputLink } from "@/lib/phases";
import { limitMarkdownLines } from "@/lib/brain-format";
...
export async function generateMetadata({ params }: { params: Promise<{ projectId: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${name} Logs`,
    description: `${name} logs and execution detail${domain ? ` for ${domain}` : ""}.`,
    openGraph: {
      title: `${name} Logs | Startup Machine`,
      description: `Execution logs and operating context${domain ? ` for ${domain}` : ""}.`,
      type: "article",
    },
    twitter: {
      card: "summary",
...
export default async function ProjectDetailPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const recommendations = Array.isArray(latestRecommendationEvent?.data?.recommendations)
    ...
    : [];
  const rowLimit = 6;
  const approvalsLimited = approvals.slice(0, rowLimit);
  const tasksLimited = tasks.slice(0, rowLimit);
  const allPacketsLimited = allPackets.slice(0, rowLimit);
  const projectAssetsLimited = projectAssets.slice(0, rowLimit);
  const recommendationsLimited = recommendations.slice(0, rowLimit);
  const nightShiftFailuresLimited = nightShiftFailures.slice(0, rowLimit);
  ...
  const chatLinks: Array<{ label: string; href: string; external?: boolean }> = [
    { label: "Phase Dashboard", href: `/projects/${projectId}/phases` },
    { label: "Current Workspace", href: `/projects/${projectId}/phases/${phaseRoute(project.phase)}` },
    { label: "Approvals Inbox", href: `/inbox?project=${projectId}` },
  ];
  if (project.live_url) {
    chatLinks.unshift({ label: "Live Landing", href: project.live_url as string, external: true });
  }
  return (
    <>
      <StudioNav active="board" pendingCount={pendingCount} />
      <main className="page studio-page studio-page-with-chat">
        <div className="studio-with-chat">
          ...
          <div>
            ...
            <p className="meta-line">{project.domain ?? "No domain"} · Logs</p>
          </div>
          <div className="table-actions">
            ...
            <Link href="/board" className="btn btn-details">
              Board
            </Link>
        ...
        <section className="studio-card">
          ...
          {nightShiftFailuresLimited.length > 0 && (
            <div className="table-shell" style={{ marginTop: 10 }}>
              <table className="studio-table compact">
                ...
                <tbody>
                  {nightShiftFailuresLimited.map((failure) => (
                    <tr key={failure.id}>
            ...
            <>
              ...
              <div className="table-shell" style={{ marginTop: 10 }}>
                <table className="studio-table compact">
                  ...
                  <tbody>
                    <tr>
                      <td style={{ width: "50%", whiteSpace: "pre-wrap", verticalAlign: "top" }}>{limitMarkdownLines(brain.mission_markdown, 6)}</td>
                      <td style={{ width: "50%", whiteSpace: "pre-wrap", verticalAlign: "top" }}>{limitMarkdownLines(brain.memory_markdown, 6)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <>
              ...
              <div className="table-shell">
                <table className="studio-table compact">
                  ...
                  <tbody>
                    {projectAssetsLimited.map((asset) => (
                      <tr key={asset.id}>
                        <td>
