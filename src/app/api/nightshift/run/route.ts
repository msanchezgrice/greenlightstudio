...
async function nightShiftHandler(req: Request) {
  ...
  for (const project of projects ?? []) {
    ...
    try {
      const jobId = await enqueueJob({
        ...
        payload: { projectId },
        idempotencyKey: `nightshift:${projectId}:${new Date().toISOString().slice(0, 10)}`,
  ...
  await enqueueJob({
    ...
    payload: { limit: 100 },
    idempotencyKey: `email:${new Date().toISOString().slice(0, 10)}`,
    priority: PRIORITY.BACKGROUND,
  });
  ...
  await enqueueJob({
    ...
    agentKey: AGENT_KEYS.SYSTEM,
    payload: {},
    idempotencyKey: `drip-digests:${new Date().toISOString().slice(0, 10)}`,
    priority: PRIORITY.BACKGROUND,
  });
  ...
  await enqueueJob({
    ...
    agentKey: AGENT_KEYS.SYSTEM,
    payload: {},
    idempotencyKey: `drip-nudges:${new Date().toISOString().slice(0, 10)}`,
    priority: PRIORITY.BACKGROUND,
  });
